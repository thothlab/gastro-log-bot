...
| `/well` | Свободная текстовая запись о самочувствии — без оценок по шкалам |
...
| `/remind` | Три дневных напоминания (по умолчанию 07:30 / 15:00 / 21:00) и расписание конкретных лекарств — в часовом поясе пользователя |
| `/stats` | Текстовая сводка и PNG-график за 7 / 30 / 90 дней |
| `/export` | ZIP-архив: `diary.txt` + CSV (симптомы, самочувствие, приёмы лекарств, еда) — для показа врачу |
